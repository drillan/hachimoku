...
class TestRunSelectorSuccess:
    ...
    async def test_returns_selector_output(self, mock_agent_cls: MagicMock) -> None:
    ...
    @patch("hachimoku.engine._selector.Agent")
    async def test_empty_selection_is_valid(self, mock_agent_cls: MagicMock) -> None:
        """空リスト返却が正常に完了する。"""
        mock_instance = MagicMock()
    ...
    @patch("hachimoku.engine._selector.Agent")
    async def test_reasoning_preserved(self, mock_agent_cls: MagicMock) -> None:
        """reasoning が保持される。"""
        mock_instance = MagicMock()
