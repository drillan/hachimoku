...
- Q: PR の入力方法はどうするか？ → A: 現在のブランチから PR を自動検出しつつ、`8moku <PR番号>` での明示指定も可能にする（後の CLI 体系決定により `--pr` から位置引数に変更）
...
4. **Given** レビュー実行中にエージェントがタイムアウトした場合, **When** タイムアウト時間を超過, **Then** 当該エージェントのみスキップされ、他のエージェントの結果は正常に出力される
...
3. **Given** エージェント定義ファイルに不正な形式のデータがある, **When** エージェントの読み込み時, **Then** 不正なエージェントのみスキップされ、エラーメッセージが表示され、他のエージェントは正常に動作する
...
2. **Given** 存在しない PR 番号を指定する, **When** `8moku 999` を実行, **Then** PR が見つからない旨のエラーメッセージが表示され、終了コード 3 で終了する
3. **Given** Issue 番号を指定する, **When** `8moku --issue 122` を実行, **Then** Issue 番号がプロンプトコンテキストに含まれ、エージェントが `gh` コマンド等で Issue 情報を自律的に取得してレビューに活用する
...
- Git リポジトリ外で実行した場合、わかりやすいエラーメッセージとともに終了する
...
- **FR-001**: システムは Git リポジトリのブランチ間差分を自動取得し、変更内容をレビュー対象として各エージェントに提供できなければならない。`8moku <PR番号>` で PR を指定した場合は、その PR の差分とメタデータを取得する
...
- **FR-009**: システムはレビュー結果の重大度に応じた終了コード（0: 問題なし, 1: Critical, 2: Important, 3: 実行エラー）を返さなければならない
...
- **FR-021**: レビューがデフォルト動作となり、`8moku` で現在ブランチのレビュー、`8moku <PR番号>` で指定 PR のレビューを実行できなければならない。`review` サブコマンドは不要とする。`init`・`agents` 等のサブコマンドは文字列で区別する
...
- **FR-025**: システムはレビュー結果を `.hachimoku/reviews/` ディレクトリに JSONL 形式で自動蓄積しなければならない（デフォルト ON）。PR レビューは `pr-{番号}.jsonl`、diff レビューは `diff.jsonl` に追記する。この動作は設定項目 `save_reviews`（デフォルト: `true`）で無効化可能とする
- **FR-026**: JSONL の各行は独立した JSON オブジェクトとし、コミットハッシュ・ブランチ名・レビュー結果・エージェント情報をメタデータとして含まなければならない
...
- **適用ルール (Applicability Rule)**: エージェントをいつ適用するかを決定する条件セット。常時適用フラグ、ファイル名パターン、差分内容パターン、実行フェーズ（early/main/final）から構成される
...
- **PR メタデータ (PR Metadata)**: GitHub PR から取得されるコンテキスト情報。タイトル・ラベル・リンク Issue で構成される。取得不可の場合は空として扱われ、レビュー自体は続行される
- **レビュー履歴 (Review History)**: `.hachimoku/reviews/` に JSONL 形式で蓄積されるレビュー結果の時系列データ。PR ごとに1ファイル（`pr-{番号}.jsonl`）、diff レビューは `diff.jsonl` に集約。各行はコミットハッシュ・レビュー結果・エージェント情報を含む独立した JSON オブジェクト
