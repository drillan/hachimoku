...
- **FR-DM-012**: システムはエージェントに許可されるツールのカテゴリを ToolCategory 列挙型（StrEnum）として定義しなければならない。現在は読み取り専用の4カテゴリ（git_read, gh_read, file_read, web_fetch）を定義する。004-configuration (FR-CF-004) の allowed_tools バリデーションと 005-review-engine (FR-RE-016) の ToolCatalog が参照する。循環依存を回避するため本仕様（002-domain-models）に配置する
...
- **ToolCategory（ツールカテゴリ）**: エージェントに許可されるツールのカテゴリを表す StrEnum 列挙型。git_read（Git 読み取り）、gh_read（GitHub 読み取り）、file_read（ファイル読み取り）、web_fetch（Web コンテンツ取得）の4カテゴリを定義する。004-configuration の allowed_tools バリデーションと 005-review-engine の ToolCatalog が参照する共有定義。循環依存回避のため 002-domain-models に配置する
