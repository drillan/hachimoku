...
- フィルタリング後の diff が空（マッチファイルなし）の場合は diff 全文を返す（フィルタリング不適用）。セレクターの選択判断を上書きしない方式
...
- Q: マッチなし時に diff 全文を返す動作はフォールバック禁止原則（Art.1）に抵触するか？ → A: 抵触しない。diff フィルタリングはオプショナルな最適化ステップであり、フィルタリング不適用時にエージェントが受け取るデータはこの機能が存在しない場合と同一である。データの改ざんやエラーの隠蔽は発生しない。Issue #152 の集約エージェントと同じ論理構造。ただしフィルタリング不適用時は warning ログを出力し、file_patterns の設定ミスを検出可能にする
