...
- 親仕様 FR-001, FR-002, FR-006, FR-007, FR-013 および US1, US2, US9 を本仕様で実現する
...
- エージェント定義（TOML）の `allowed_tools` にツールカタログに存在しないカテゴリ名が指定されている場合、当該エージェントは読み込みエラーとして LoadResult に記録され、レビュー実行から除外される

## Requirements *(mandatory)*

### Functional Requirements

- **FR-RE-001**: システムは入力モード（diff / PR / file）に応じてレビュー指示情報を構築し、エージェントのプロンプトコンテキストとして提供しなければならない（親仕様 FR-001）。実際の差分取得・ファイル読み込み・PR 情報取得は、エージェントが `allowed_tools`（git, gh 等）を使って自律的に実行する:
  - **diff モード**: `base_branch` 名とレビュー指示（マージベースからの差分をレビューすること）をプロンプトコンテキストに含める。エージェントが git ツールを使って差分を取得する。推奨される差分取得方法（merge-base からの差分）はシステムプロンプトでガイドするが、エージェントは状況に応じてコマンドオプションを柔軟に選択できる
  - **PR モード**: PR 番号とレビュー指示をプロンプトコンテキストに含める。エージェントが gh/git ツールを使って PR 差分とメタデータ（タイトル・ラベル・リンク Issue）を自律的に取得する
  - **file モード**: 指定されたファイルパス（単一ファイル・複数ファイル・ディレクトリ・glob パターン）をプロンプトコンテキストに含める。エージェントがファイル読み込みツールを使ってファイル内容を取得する
...
- **FR-RE-002**: システムは以下の手順でエージェント実行パイプラインを構成しなければならない（親仕様 FR-002）:
  ...
  4. **レビュー指示構築**: 入力モード（diff / PR / file）に応じてレビュー指示情報を構築する
  5. **エージェント選択（セレクターエージェント）**: セレクターエージェント（pydantic-ai エージェント）を実行し、レビュー指示情報と利用可能なエージェント定義一覧を渡す。セレクターエージェントは git/gh ツールでレビュー対象を調査し、実行すべきエージェントリストを構造化出力で返す。エージェント定義の `file_patterns` / `content_patterns` / `phase` はセレクターの判断ガイダンスとして提供される
  6. **実行コンテキスト構築**: 選択された各エージェントに対して実行コンテキスト（モデル・ツール・プロンプト・タイムアウト・最大ターン数）を構築する
  7. **エージェント実行**: 逐次または並列でエージェントを実行する
  8. **結果集約**: 各エージェントの結果（AgentResult）を ReviewReport に集約する
...
- **FR-RE-016**: システムは以下のツールカタログを定義し、全エージェント（セレクターエージェント・レビューエージェント）が使用できるツールをカテゴリベースで管理しなければならない:
  ...
  エンジンは以下のガードレールを強制する:
  - エージェント定義（TOML）の `allowed_tools` に含まれるツール名をカタログに対してバリデーションする
  - カタログに存在しないツール名が指定された場合、当該エージェント定義はバリデーションエラーとし、LoadResult のエラー情報に記録する（FR-RE-014 と同様の伝達方法）
...
- **ReviewTarget（レビュー対象）**: レビュー対象を抽象化したエンティティ。入力モード（diff / PR / file）に応じて、変更ファイルリスト・差分内容またはファイル全体内容・PR メタデータ（PR モード時のみ）を保持する。エージェントセレクターとエージェント実行の両方で参照される
...
- **SelectorAgent（セレクターエージェント）**: pydantic-ai エージェントとして実行されるエージェント選択処理。レビュー指示情報と利用可能なエージェント定義一覧（名前・説明・適用ルール・フェーズ）をプロンプトで受け取り、git/gh ツールでレビュー対象を調査した上で、実行すべきエージェント名のリストをフェーズ順で構造化出力として返す。003-agent-definition の `file_patterns` / `content_patterns` は判断ガイダンスとして機能する。`allowed_tools` は全読み取りカテゴリ（`git_read`, `gh_read`, `file_read`）を使用する
...
- 全ての git/gh コマンドは LLM エージェントが `allowed_tools` 経由で自律的に実行する。hachimoku 本体が subprocess でメカニカルに実行することはしない。`git` および `gh` コマンドがエージェント実行環境で利用可能であることを前提とする
