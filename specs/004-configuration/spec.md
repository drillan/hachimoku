...
開発者が設定ファイルでセレクターエージェント（エージェント選択用 pydantic-ai エージェント）のモデル・タイムアウト・最大ターン数を上書きする。セレクターエージェント設定は `[selector]` セクションで管理される。Issue #64 で導入。Issue #118 で `allowed_tools` を SelectorDefinition（TOML 定義）に移管。

**Why this priority**: セレクターエージェントのカスタマイズは運用上の柔軟性を提供するが、まず設定ファイルの読み込みと階層解決（P1）が安定してから取り組む。セレクターのデフォルト設定でも動作するため、優先度は P2。

**Independent Test**: 設定ファイルの `[selector]` セクションでモデルやタイムアウト・最大ターン数を指定し、設定モデルの `selector` フィールドに正しく反映されることで検証可能。

**Acceptance Scenarios**:

1. **Given** 設定ファイルに `[selector]` セクションが存在しない, **When** 設定を解決する, **Then** セレクター設定はデフォルト値（model/timeout/max_turns はグローバル値）で構成される
...
- `[selector]` セクションが存在しない場合、デフォルト値（model/timeout/max_turns はグローバル値）が適用される
...
- **FR-CF-010**: システムはセレクターエージェント設定（`[selector]` セクション）を管理しなければならない。セレクター設定は model, timeout, max_turns の3項目を持ち、いずれも未設定の場合は SelectorDefinition の値またはグローバル設定値が適用される。セレクター設定の階層解決は FR-CF-007 に従い項目単位で行われる（Issue #64 で導入、Issue #118 で `allowed_tools` を SelectorDefinition に移管）
...
- **SelectorConfig（セレクターエージェント設定）**: セレクターエージェント（エージェント選択用 pydantic-ai エージェント）の設定上書き情報を表す不変モデル。モデル上書き（model）、タイムアウト上書き（timeout）、最大ターン数上書き（max_turns）を持つ。全フィールドはオプショナルで、未設定の場合は SelectorDefinition の値またはグローバル値が適用される。Issue #64 で導入、Issue #118 で `allowed_tools` を SelectorDefinition に移管
...
- セレクターエージェントの `allowed_tools` は SelectorDefinition（TOML 定義）で管理される（Issue #118 で SelectorConfig から移管）。ToolCategory 列挙型は引き続き 002-domain-models で定義
