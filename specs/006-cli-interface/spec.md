...
- 親仕様 FR-008（CLI 部分: 出力形式選択）、FR-009、FR-016、FR-017、FR-018（CLI 部分: ストリーム分離の最上位制御）、FR-021、FR-027、FR-028、FR-029、FR-031、FR-032 および US7、US8、US9（CLI 部分）を本仕様で実現する
...
- 親仕様 FR-021 のサブコマンド文字列に `config` が含まれるが、v0.1 では `init` と `agents` のみ実装する。`config` サブコマンドは将来の拡張として予約する
...
8. **Given** `--issue 122` を指定する状態, **When** レビューを実行する, **Then** Issue 番号がエージェントのプロンプトコンテキストに含まれる
9. **Given** `--show-cost` を指定する状態, **When** レビューが完了する, **Then** コスト情報がレポートに含まれる
...
3. **Given** 特定のエージェント名を指定する状態, **When** `8moku agents code-reviewer` を実行, **Then** code-reviewer の全設定情報（説明・モデル・フェーズ・出力スキーマ・適用条件・許可ツール）が表示される
...
- file モードで `.hachimoku/` ディレクトリが見つからない場合（Git リポジトリ外での実行を含む）、デフォルト設定（ビルトインエージェント）でレビューを実行する（警告メッセージを stderr に表示）
...
- **FR-CLI-002**: システムは位置引数からレビューモードまたはサブコマンドを以下の優先順で自動判定しなければならない（親仕様 FR-021）:
  1. **サブコマンド文字列**（`init`, `agents`）→ 対応するサブコマンドを実行
...
- **FR-CLI-003**: システムはレビュー結果の重大度に応じた以下の終了コードを返さなければならない（親仕様 FR-009）:
  ...
  - **3**: 実行エラー（全エージェント失敗）
  - **4**: 入力エラー（PR 未発見・Git リポジトリ外での diff/PR 実行・ファイル未発見・引数解析エラー・`gh` 未認証等）
...
- `config` サブコマンドは v0.1 では実装しない。Typer に予約サブコマンドとして登録せず、将来の拡張として位置づける
