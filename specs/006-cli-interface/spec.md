...
- `8moku init` は Git リポジトリの内外を問わず動作する。Git リポジトリ内（`.git` ディレクトリが存在する場合）では `.gitignore` への `/.hachimoku/` エントリ追加も行う。Git リポジトリ外では `.gitignore` 操作をスキップする（警告なし）
...
4. **Given** Git リポジトリ外で実行する, **When** `8moku init` を実行, **Then** `.hachimoku/` 配下に設定ファイル・エージェント定義・レビューディレクトリが生成される。`.gitignore` への操作はスキップされる（警告なし）
...
- diff モード・PR モードにおいて Git リポジトリ外で実行した場合、エラーメッセージとともに終了コード 4 で終了する（file モードおよび init サブコマンドは Git リポジトリ外でも動作可能）
...
- **FR-CLI-007**: `8moku init` コマンドは以下を実行しなければならない（親仕様 FR-027）:
  1. `.hachimoku/` ディレクトリを作成する
  2. `.hachimoku/config.toml` を生成する（全設定項目をコメントとして記載したテンプレート）
  3. `.hachimoku/agents/` にビルトイン6エージェントの定義ファイルをコピーする（003-agent-definition のパッケージ内バンドルから取得）
  4. `.hachimoku/reviews/` ディレクトリを作成する
  5. Git リポジトリ内（`.git` ディレクトリが存在する場合）のみ、`.gitignore` に `/.hachimoku/` エントリを追加する
...
- **FR-CLI-009**: file モードの CLI は以下の入力形式をサポートしなければならない（親仕様 FR-029）:
  ...
  - **拡張子フィルタ**: `--ext .py --ext .rst` でファイルモードの対象ファイルを拡張子で絞り込む。`config.toml` の `file_extensions` でプロジェクトデフォルトを設定可能。CLI `--ext` は config 値を完全に上書きする。`.py` と `py` の両形式を受け付け、内部でドット付き小文字に正規化する。未指定時は全ファイルを対象とする（現行動作を維持）
...
- **InitHandler（init ハンドラー）**: `8moku init` コマンドのビジネスロジック。`.hachimoku/` ディレクトリ構造の作成、設定テンプレート生成、ビルトインエージェント定義のコピー（003-agent-definition のパッケージ内バンドルから）を担当する。Git リポジトリ内では `.gitignore` への `/.hachimoku/` エントリ追加も行う。`--force` オプションによる上書き制御を含む
