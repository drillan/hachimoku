...
**重要:** Task 4 はファイル削除をステージするのみでコミットしない。Task 4 単独ではリポジトリはビルド不可状態になる。Task 4・Task 5 を連続実行し、Task 5 Step 7 の単一コミットでビルド可能状態を回復させる（Task 4 で独立コミットを作らないこと）。
...
**変更:** `src/hachimoku/cli/_app.py`、`src/hachimoku/cli/_history_writer.py`、`pyproject.toml`、`CLAUDE.md`
...
| 対象 | 更新要否 | 内容 |
|------|---------|------|
| `specs/008+/spec.md` | 否（後続） | SpecKit 形式の spec は設計書 §15 のドキュメント影響表に「specs/008+ 新規作成」として登録済み。移行のドキュメント整備の一環として別途作成する（下記「仕様トレーサビリティ」参照） |
| `docs/ja`・`docs/en` | 否（後続） | `cli.md` 等の利用者向け改訂は設計書 §15 のとおり後続。SP1-1 はエンジン撤去のみで、利用者向け CLI の最終形は SP1-2 以降で確定する |
| `README.md` | 否（後続） | 同上 |
| `CLAUDE.md` | 要 | Task 6 で Tech Stack / Active Technologies から pydantic-ai・claudecode-model 記述を除去（設計書 §15） |

## 仕様トレーサビリティ

本計画は設計書 `docs/superpowers/specs/2026-05-18-skill-migration-design.md` を
上位仕様とする。プロジェクト CLAUDE.md 開発ワークフロー Phase 1 が要求する
`specs/NNN-xxx/spec.md`（SpecKit 形式）は、設計書 §15 のドキュメント影響表に
登録済みであり、移行のドキュメント整備として別途作成する。本計画の段階では
設計書が仕様の役割を担う。
...
- タグ名（既定案: `v0.0.41`）
- 軽量 Release を作成してよいか
- タグ対象コミット（既定: 設計書 §13 が指定する移行前最終コミット `c01682d`、バージョン 0.0.41）

ユーザーが承認するまで Step 2 以降に進まない。

- [ ] **Step 2: 対象コミットを検証し注釈付きタグを作成する**

Run（対象コミットの存在確認）:
```bash
git rev-parse c01682d
```
Expected: `c01682d` で始まる完全な SHA が表示される。

Run（タグ作成。対象コミットを明示する）:
```bash
git tag -a v0.0.41 c01682d -m "Last claude -p based release before Agent Skills migration"
```
...
- [ ] **Step 4: 抽出値が現行の原本と一致することを検証する**

`engine/_tools/_git.py` の `ALLOWED_GIT_SUBCOMMANDS`、`engine/_tools/_gh.py` の
`ALLOWED_GH_PATTERNS` および `_IMPLICIT_POST_FLAGS` を実際に開き、
`security/readonly_allowlist.py` の3定義と要素が完全一致することを確認する。
不一致があれば**原本を正として** `readonly_allowlist.py` と Step 1 のテスト
期待値を修正する。原本（`engine/_tools/`）は Task 4 で削除されるため、この
検証は Task 4 より前に必ず行う。
...
プロジェクトルートの `CLAUDE.md` を開き、Tech Stack 節と「## Active Technologies」
節から `pydantic-ai` / `pydantic-graph` / `claudecode` 関連の記述を削除する。
`pydantic`（v2）と `typer` は残す。これは設計書 §15「CLAUDE.md（Tech Stack から
pydantic-ai 除去）= 要」に対応する最小限の事実訂正であり、`CLAUDE.md` の全面改訂は
移行完了時に別途行う。

- [ ] **Step 5: 全テストと静的検査を確認する**
...
- SP2 / SP3: 設計書 §12 のとおり。`build` コマンドは SP2（変換パイプライン）で導入する。

`agents` コマンドは現行から存続し、本計画でも温存する（縮小後の CLI は `init` / `agents`）。
