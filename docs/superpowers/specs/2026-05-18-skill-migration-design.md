...
hachimoku は 2 つの配布物に分かれる。

- **薄い CLI**（Python パッケージ）: `select` / `aggregate` / `build` / `init`。
- **Claude Code プラグイン**: オーケストレーター skill・サブエージェント・hook。

ユーザーの導入は **Claude Code プラグインの導入のみ**を基本とする。プラグインの
オーケストレーター skill は、薄い CLI を `uvx` でエフェメラル実行する:

```text
uvx --from git+https://github.com/drillan/hachimoku@v<plugin版> hachimoku select ...
```

- バージョンはプラグイン版にピン留めする（プラグイン同梱の `manifest.json` と
  薄い CLI の manifest 解釈を一致させるため）。
- `uv` / `uvx` が PATH に無い場合、skill は明示的にエラーを出し（フォールバック
  せず）、`uv` の導入手順を案内する。
- skill はレビュー実行時に、現状の `uvx` ブリッジが移行期の方式である旨を 1 行
  warning として出す。
- `uv tool install` は任意。薄い CLI を恒常的に PATH へ置きたい（`8moku build`
  を手元で多用する等）ユーザー向けの補助手段であり、必須ではない。

`uv tool install` だけではレビューは実行できない（レビュー起動コマンドは撤去
済み）。レビューは Claude Code セッション内でプラグイン経由で行う。

**将来方向**: 薄い CLI をプラグイン配布物へ同梱し、`uvx` ブリッジを廃して
「プラグイン導入のみ」に一本化する（v1 ではスコープ外）。
...
- **PyPI 公開可否**: `uvx` は git URL ピン留めで動作するため PyPI 公開は必須では
  ない。公開すれば `uvx hachimoku==<版>` と簡潔化できる。SP3 で判断する。
