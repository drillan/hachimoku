...
ビルトインエージェントを標準提供し、プロジェクト固有のカスタムエージェントも追加できます。
`hachimoku build` はエージェント TOML 定義を Claude Code サブエージェント用の `.md` ファイルに変換します。
...
|---------------|------|-------------|---------|---------|
| architecture-reviewer | アーキテクチャ・設計パターンの問題検出（SOLID 原則、レイヤー違反、循環依存、結合度） | severity_classified | main | content_patterns |
| breaking-change-detector | 公開 API・データモデル・CLI・設定ファイルの破壊的変更検出 | severity_classified | main | content_patterns |
| code-reviewer | コード品質・バグ検出 | scored_issues | main | 常時適用 |
| dependency-auditor | 依存関係のセキュリティ・健全性監査 | severity_classified | main | content_patterns |
| performance-analyzer | パフォーマンス問題の検出（計算量、メモリ効率、I/O、リソースリーク） | severity_classified | main | content_patterns |
| plan-reviewer | 実装プラン文書の品質・Issue/ADR/仕様との整合性レビュー | scored_issues | main | file_patterns + content_patterns |
| pr-test-analyzer | テストカバレッジの評価 | test_gap_assessment | main | file_patterns |
| security-analyzer | セキュリティ脆弱性の検出（インジェクション、認証欠陥、シークレット漏洩、OWASP Top 10） | severity_classified | main | content_patterns |
...
```{note}
**セレクションと集約は決定論的な CLI ステップであり、LLM エージェントではありません。**
`hachimoku select` と `hachimoku aggregate` は、LLM バジェットを消費せずに、
実行すべきエージェントの決定と出力のマージを行います。
`selector.toml` や `aggregator.toml` のビルトインエージェント定義は存在しません。
これらのステップを制御する `[selector]` および `[aggregation]` 設定セクションについては
...
| `max_turns` | `int \| None` | No | エージェント固有の最大ターン数。デフォルト: `None`（グローバル設定を使用）。正の値 |
| `timeout` | `int \| None` | No | エージェント固有のタイムアウト秒数。デフォルト: `None`（グローバル設定を使用）。正の値 |
...
| エージェント | 条件 | パターン |
|------------|------|---------|
| architecture-reviewer | content_patterns | `class\s+\w+\(`, `def\s+__init__`, `@abstractmethod`, `@overload`, `ABC\)`, `Protocol\)`, `trait\s+\w+`, `impl\s+\w+\s+for`, `interface\s+\w+`, `extends\s+\w+` 等 |
...
| performance-analyzer | content_patterns | `\.execute\(`, `query\(`, `\.fetchall\(`, `\.read\(`, `\.write\(`, `sleep\(`, `Lock\(`, `async\s+def`, `await\s+`, `Mutex`, `batch` 等 |
| plan-reviewer | file_patterns + content_patterns | ファイル: `*.md` / コンテンツ: `^#\s+Issue\s+#\d+`, `受入条件\|Acceptance Criteria`, `実装ステップ\|Implementation Steps?` |
