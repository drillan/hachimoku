...
Sync Impact Report
==================
Version change: 0.0.0 → 1.0.0 (新規作成)
...
# hachimoku Constitution

## Core Principles

### Article 1: Test-First Imperative（テストファースト原則）
...
### Article 2: Documentation Integrity（ドキュメント整合性）
...
### Article 3: CLI Design Compliance（CLI 設計準拠）

すべてのCLI実装は、設計原則に準拠しなければならない（MUST）。

**CLI要件**:
1. すべてのコマンドは`--help`で明確な使用方法を表示しなければならない（MUST）
2. エラーメッセージは具体的な解決方法を含めなければならない（MUST）
3. 終了コードは成功（0）と失敗（非0）を適切に返さなければならない（MUST）
4. レビューレポートは stdout に出力し、進捗表示・ログは stderr に出力しなければならない（MUST）

**データ駆動型アーキテクチャ要件**:
- エージェント定義は TOML ファイルで外部化しなければならない（MUST）
- エージェントは定義ファイルの追加のみで拡張可能でなければならない（MUST）
- 設定は `.hachimoku/config.toml` で一元管理しなければならない（MUST）

**理由**: CLIの一貫した使用体験と、コード変更なしのエージェント拡張性を保証

### Article 4: Simplicity（シンプルさ）
...
### Article 5: Code Quality Standards（コード品質基準）
...
### Article 6: Data Accuracy Mandate（データ正確性）

**これは非交渉的である**: すべてのデータは、明示的なソースから取得しなければならない（MUST）。
推測、暗黙的デフォルト値、ハードコードは一切認めない。
...
### Article 8: Refactoring Policy（リファクタリングポリシー）
...
### Article 9: Python Type Safety Mandate（Python型安全性）
...
async def run_agent(definition: AgentDefinition) -> AgentResult:
    """エージェントを実行し、結果を返す"""
    ...

# 悪い例（禁止）
def run_agent(definition):  # 型注釈なし
    return definition
```

**理由**: 静的型チェックによりバグの早期発見、コード可読性向上、リファクタリング安全性を保証

### Article 10: Python Docstring Standards（Python Docstring標準）
...
### Article 11: Naming Convention Compliance（命名規則準拠）

**これは非交渉的である**: すべてのブランチ名、コミットメッセージ、ディレクトリ名は、標準化された命名規則に従わなければならない（MUST）。

**参照**: 命名規則の詳細は `.claude/git-conventions.md` で定義される。

**原則**:
- 一貫性: プロジェクト全体で統一された命名を使用する
- 識別性: 名前から目的が明確にわかる
- トレーサビリティ: issueとの関連付けが可能
...
- 例: `001-architecture-spec`, `002-domain-models`

注: Gitブランチはゼロパディングなし（`.claude/git-conventions.md` 参照）

**理由**: プロジェクトの一貫性と識別性を保証
