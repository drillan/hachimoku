...
| レビューモード | 蓄積先ファイル | メタデータ |
|------------|-------------|----------|
| diff | `diff.jsonl` | コミットハッシュ、ブランチ名 |
| PR | `pr-{番号}.jsonl` | コミットハッシュ、PR 番号 |
| file | `files.jsonl` | ファイルパスリスト、実行日時、作業ディレクトリ |
